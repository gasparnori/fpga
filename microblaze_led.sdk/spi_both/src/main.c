...
void initADC(){
	...
	GPIO_2_conf.BaseAddress = XPAR_GPIO_2_BASEADDR;
	GPIO_2_conf.DeviceId = XPAR_GPIO_2_DEVICE_ID;
	...
	XGpio_SetDataDirection(&ADC_control, 2, 0);
	XGpio_SetDataDirection(&ADC_control, 1, 1);
	XGpio_DiscreteWrite(&ADC_control, 2, 0);
}
void WriteRGB (u32 color){
...
u8 SPI_t(XSpi *SpiInstancePtr, u8 message){
	...
	XSpi_Transfer(SpiInstancePtr, &message, &received, 3);
...
void startADC(){
	 XGpio_DiscreteWrite(&ADC_control, 2, 0xFF);
}
int main() 
{
   Xil_ICacheEnable();
   ...
   while(1){
	...
	//if data available
	if (XGpio_DiscreteRead(&ADC_control, 2) == 0){
		WriteLED(1);
		SPI_t(&MasterInstance, 0xFF);
		//printf("sent\r\n");
	}
	//extBtn();
   }
