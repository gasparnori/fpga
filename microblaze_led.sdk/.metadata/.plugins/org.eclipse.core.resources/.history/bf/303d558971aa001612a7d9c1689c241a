...
int interpret_command(u8 command){
	if (command==240){
		WriteRGB(PINK);
		return 1;
	}
...
int main() 
{
   ...
   while(1){
		for (buf_counter=0; buf_counter < BUFFER_SIZE; buf_counter++){
			    	...
			    	u8 ans=answer(ReadBuffer[prev_command]);
					ReadBuffer[buf_counter]=SPI_t(&SpiInstance, ans);
					interpret_command(ReadBuffer[buf_counter]);
					//xil_printf("received: %d , sending: %d \r\n", ReadBuffer[prev_command], ans);
		}
